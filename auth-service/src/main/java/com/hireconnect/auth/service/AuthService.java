...
public class AuthService {
    ...
    public void initAdmin() {
        if (!repo.existsByEmailIgnoreCase("admin@gmail.com")) {
            UserCredential admin = UserCredential.builder()
                    .email("admin@gmail.com")
                    .passwordHash(passwordEncoder.encode("12345678"))
                    .role(com.hireconnect.auth.domain.UserRole.ADMIN)
    ...
    public AuthResponse issueTokens(UserCredential user) {
    ...
    public void suspendUser(Long id) {
        UserCredential user = repo.findById(id).orElseThrow(() -> new IllegalArgumentException("User not found"));
