package com.hireconnect.job.api.error;
...
public class GlobalExceptionHandler {
    ...
    public ResponseEntity<ApiError> validation(MethodArgumentNotValidException ex, HttpServletRequest req) {
        log.warn("Validation error: {} on {}", ex.getMessage(), req.getRequestURI());
    ...
    public ResponseEntity<ApiError> illegalArgument(IllegalArgumentException ex, HttpServletRequest req) {
        log.warn("Illegal argument: {} on {}", ex.getMessage(), req.getRequestURI());
        ApiError body = ApiError.builder()
                .timestamp(Instant.now())
                .status(HttpStatus.BAD_REQUEST.value())
                .error("Bad Request")
